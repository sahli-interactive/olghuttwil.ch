...
function Header({ blok }) {
  return (
    <header className={`relative p-6 bg-yellow${blok.picture?.filename ? ' h-screen' : ''}`}>
      ...
      <div className={`mt-16 self-end col-span-6 bg-yellow rounded-r-50 z-10  flex flex-col h-fit gap-4 bottom-0 left-0 ${blok.picture?.filename ? 'p-12' : 'px-0 py-12'}`}>
        {blok.headline && <h1 className="h1 text-4xl font-bold">{blok.headline}</h1>}
        {blok.lead && <p className="text-lg mb-9">{blok.lead}</p>}
