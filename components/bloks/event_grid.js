            ...
            <div className="grid grid-cols-12 gap-4 mt-6 flex-wrap container">
                <div className="col-span-12 col-start-1">
                    ...
                    <div className="flex flex-wrap gap-4 mt-8 mb-4 overflow-hidden">
                        <button onClick={() => setActiveTag(null)} className={`px-6  py-3 whitespace-nowrap rounded-full flex inline-flex items-center bg-white hover:bg-yellow text-blue-700 font-semibold ${activeTag === null ? ' bg-yellow border-3 border-yellow' : ' border-blue-700 border-3'}`}>Alle</button>
                        {blok.tags && blok.tags.map((tag, index) => (
                            <button
                                key={index}
                                onClick={() => setActiveTag(tag)}
                                className={`px-6 py-4 whitespace-nowrap rounded-full flex inline-flex items-center bg-white hover:bg-yellow text-blue-700 font-semibold ${tag === activeTag ? 'bg-yellow border-3 border-yellow' : ' border-blue-700 border-3'}`}
                            >
                                {tagStrings[tag]}
                            </button>
                        ))}
